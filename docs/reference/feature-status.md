---
title: Feature status and operational boundaries
---

# Feature status and operational boundaries

## Ready to document

The source includes the public forms, protected CRM routes, dashboard, leads, tasks, site visits, lots, applications, customers, contracts, payments, collections, reports, settings, audit trail, and feedback controls.

## Implemented but verify live

Application-to-lead automation, public-inquiry routing, reservations/deposit workflow, post-sales work, AI summaries, Daily Brief, Email Center, and privileged data management need authorized test-account confirmation of deployed migrations, Edge Functions, permissions, secrets, and business rules.

## Partial boundaries

Reservations do not replace the ledger; Email Center is a manual outbox, not a full inbox/automated campaign; AI is advisory; Daily Brief emailing is not active.

## Planned or unavailable

Customer portal/login, customer payment-proof upload, customer document downloads, automated email/WhatsApp, scheduled Daily Brief delivery, customer notification preferences, and customer-facing AI are not current features.
